...
def train_lstm_model(**kwargs):
    """
    Train an LSTM model on financial data and save model checkpoint and scaler with UUID-based names.
    
    Args:
        **kwargs: Airflow task instance context with parquet_path from ETL pipeline.
    
    Returns:
        dict: Paths to saved model checkpoint and scaler.
    """
    ti = kwargs['ti']
...
def metric_and_predict_lstm_model(**kwargs):
    """
    Evaluate LSTM model, compute RMSE metrics, and predict next close price.
    
    Args:
        **kwargs: Airflow task instance context with parquet_path and model/scaler paths.
    
    Returns:
        str: Path to the prediction output file.
    """
    ti = kwargs['ti']
    parquet_path = ti.xcom_pull(task_ids='transform_data', dag_id='etl_pipeline')
